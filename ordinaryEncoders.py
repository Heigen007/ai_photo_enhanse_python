...
RESULTS_FILE = "jpeg_results_with_time.csv"  # Updated results file
...
# Функция сохранения изображения в JPEG с замером времени
def save_compressed_images(image_path):
    ...
    compression_time = time.time() - start_time  # End timing
    
    return jpeg_path, compression_time
...
for img_path in image_files:
    jpeg_path, compression_time = save_compressed_images(img_path)
    psnr_jpeg, ssim_jpeg, orig_size, jpeg_size = calculate_metrics(img_path, jpeg_path)
    
    if psnr_jpeg is not None:
        results.append([os.path.basename(img_path), "JPEG", psnr_jpeg, ssim_jpeg, orig_size, jpeg_size, compression_time])
...
# Вычисление средней скорости сжатия для JPEG
average_compression_time = results_df["Compression Time (s)"].mean()
print(f"Результаты сохранены в {RESULTS_FILE}")
print(f"Среднее время сжатия для JPEG: {average_compression_time:.4f} секунд")